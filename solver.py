...
class Solver(object):
    ...
    def build_model(self):
        self.model = TimeDetector(win_size=self.win_size, enc_in=self.input_c, c_out=self.output_c, n_heads=self.n_heads, d_model=self.d_model, e_layers=self.e_layers, patch_size=self.patch_size, channel=self.input_c)
    ...
    def vali(self, vali_loader):
        ...
        for i, (input_data, _) in enumerate(vali_loader):
            ...
            trend, residual = self.model(input)
            loss = self.model.train_vai_loss(trend, residual)
    ...
    def test(self):
        ...
        for i, (input_data, labels) in enumerate(self.train_loader):
            input = input_data.float().to(self.device)
            trend, residual = self.model(input)
            trend_loss = self.model.test_loss(trend, residual)
